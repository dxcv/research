...
def make_pdf(dist, params, size=10000):
    ...
    # ppf is the percent point function, i.e. inverse cdf
...
def make_best_dist_plot(data, bins):

    best = best_fit_distribution(data, bins)
    best_dist = getattr(st, best[0])
    pdf = make_pdf(best_dist, best[1])
    ax = pdf.plot(lw=2, label='PDF', legend=True)
    data.dropna().plot(kind='hist', bins=12, density=True, alpha=0.5, label='Data', legend=True, ax=ax)